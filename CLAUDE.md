...
- **Difficulty** — `clearLines()` owns scoring *and* progression: `level = startLevel + floor(lines/10)`, `dropInterval = max(100, 1000 - (level-1)*90)`. `startLevel` defaults to `1`, so this reduces to the original `floor(lines/10)+1` unless the pause menu's level selector changed it. Do not go back to a bare `floor(lines/10)+1` here — that silently discards the chosen starting level on the very first line clear.
...
- **Pause menu (`#pause-menu`)** — a sibling overlay to `#overlay` (Game Over keeps its own; they are never shown at once because `togglePause()` bails out early when `gameOver`). `togglePause()` toggles `paused` and shows/hides `#pause-menu`; it no longer writes to `#overlay`. Both `P` and `Escape` call `togglePause()` from the keydown handler, ahead of the `if (paused || gameOver) return;` guard that blocks gameplay keys while the menu is open. Buttons: `#pm-resume`, `#pm-restart`, `#pm-controls-toggle` (shows/hides the `#pm-controls` key list), and the `#pm-level` `<select>` (populated 1–15 at load from `MAX_START_LEVEL`). Every button handler calls `blur()` on itself before doing anything else — otherwise a focused button "reclicks" itself on the next `Space`/`Enter` after resuming, eating a piece. Resuming resets `lastTime = performance.now()` before calling `loop()` again, same invariant as the old pause path.
- **Start level** — `#pm-level`'s choice is persisted in `localStorage` under `tetris.startLevel` (`loadStartLevel`/`saveStartLevel`), validated by `isValidStartLevel` (`Number.isInteger`, range 1–`MAX_START_LEVEL`) so a corrupt or missing value falls back to 1 without breaking startup. It only affects the *next* game: `init()` reads the module-scope `startLevel` into `level` and derives `dropInterval` from it with the same formula `clearLines()` uses, and also closes `#pause-menu`.
...
- `#restart-btn` (Game Over overlay) and `#pm-restart` (pause menu) both just call `init()` — there is only one restart path, don't duplicate its logic.
- Any new button inside `#pause-menu` must `blur()` itself in its click handler (see the invariant above) or a stray `Space`/`Escape`/`Enter` right after resuming will re-trigger it.
